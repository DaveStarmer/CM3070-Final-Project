...
class MovementDetection:
    ...
    """
    def __init__(self, frame_rate=30, detection_threshold=50, alert_contour_area=10000,
                 video_preroll=3, video_length=20):
        ...
        self._video = []
        # detection instance
        self._bg_sub = cv2.createBackgroundSubtractorMOG2(history=FRAME_HISTORY,varThreshold=25)
    ...
    def add_frame(self, frame):
        ...
        if self._captured_frames >= self.video_length * self._frame_rate - self.preroll:
            self._tidy()
            return True
        self._tidy()
        return False
    ...
    def _tidy(self):
        frames_to_retain = int(self._frame_rate * (self.video_length + ADDITIONAL_PROCESSING_SECS))
        if len(self._video) > frames_to_retain:
            self._video = self._video[-frames_to_retain:]

    def get_video(self):
        # if (datetime.now() - self._movement_dt).total_seconds() - self.preroll >= self.video_length:
        self._movement_dt = None
        self._captured_frames = 0
        return self._video
